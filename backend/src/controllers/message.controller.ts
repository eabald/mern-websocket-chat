...
class MessageController implements Controller {
  ...
  public initializeWebsocket(server: Server) : void {
    ...
    this.websocket.on('connection', (socket: SocketWithUser) => this.bindSocketEvents(socket, this.websocket));
  }

  // workout better naming convention
  private bindSocketEvents(socket: SocketWithUser, websocket: socketio.Server) : void {
    this.rejoinUserRooms(socket)
    socket.on('messageSent', message => this.messageReceived(message, socket, websocket));
  }
  ...
  private async messageReceived(message: Message, socket: SocketWithUser, websocket: socketio.Server) : Promise<void> {
    ...
    usersInRoom.forEach(user => {
      const userSocket: socketio.Socket = websocket.sockets.sockets.get(user.socketId);
